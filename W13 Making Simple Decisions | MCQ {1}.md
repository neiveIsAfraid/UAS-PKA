...
A) Minimizing maximum possible loss (Minimax).
B) Maximizing the probability of reaching a goal state.
C) Maximizing the expected utility (MEU) of the action outcomes.
D) Satisfying all transitive preferences.
...
C) Utility node (diamond)
D) Evidence node (circle)

**Question 5**
What is the definition of **Stochastic Dominance** in the context of multiattribute utility theory?
A) A situation where one option is superior on all relevant attributes.
...
C) The property that a utility function must be linear with respect to the amount of money involved.
D) The act of making an immediate decision based on estimated utility rather than seeking further information.

**Question 6**
What is the primary purpose of the **Utility Function U(s)** in decision theory?
A) To assign a probability distribution over the possible state transitions.
B) To compute the total cost of acquiring evidence.
C) To assign a single numerical value representing the desirability of a state.
D) To quantify the agent's degree of belief in a given state.

**Question 7**
Which property holds true for the **Expected Value of Information (VPI)**, according to a theorem derived from its definition?
A) VPI is always negative, representing a cost incurred.
B) VPI is guaranteed to converge to the utility of the optimal action.
C) VPI is always non-negative.
D) VPI equals the cost of the optimal action sequence.
...
B) The contrast between expected utility and immediate, realized utility.
C) The contrast between deterministic outcomes and uncertain outcomes.
D) The contrast between single-attribute and multiattribute utility functions.
...
A) Normative theory describes how actual agents act, while descriptive theory describes how a rational agent should act.
...
C) Normative theory applies only to continuous variables, while descriptive theory applies only to discrete variables.
D) Descriptive theory uses probabilities, while normative theory uses logic.

**Question 12**
What does the **Value of Information (VPI)** specifically measure?
A) The expected cost of gathering information before making a decision.
...
C) The actual utility gained after successfully acquiring and using new evidence.
D) The marginal probability of observing a new piece of evidence $E_j$.
...
D) Myopic information gathering
...
A) The human is guaranteed to be more rational than the robot.
B) Deference costs the robot less computation time.
...
D) The action of being switched off has a guaranteed utility of zero.

***

## Solutions Section

[This is the answers](https://github.com/neiveIsAfraid/UAS-PKA/blob/main/W13%20Making%20Simple%20Decisions%20%7C%20MCQ%20Answers%20%7B1%7D.md)
